...
//等待缓冲区读取完成
static void wait_for_finish(buffer_head* bh)
{
...
{
    if (!blk_devs[BLKDEV_MAJOR(dev)].do_request) return NULL;
...
{
    if (!bh) return -ENOBUFS;
    if (count + pos > BLOCK_SIZE) count = BLOCK_SIZE - pos;   // truncate
...
{
    if (!blk_devs[bh->dev].do_request) return -ENODEV;
    if (!bh->dirt) return 1;
    make_request(bh->dev,
                 BLKDEV_REQ_WRITE,
                 bh->blocknr * BLOCK_SIZE / SECTOR_SIZE,
...
{
    if (bh->dev == -1 || !blk_devs[bh->dev].do_request) return -ENODEV;

    if (bh->uptodate) return 1;   //数据没变，可以直接返回
    //这里的疑点是，可以直接返回吗？
    //应该要等数据准备完毕之后再返回的
    make_request(bh->dev,
                 BLKDEV_REQ_READ,
...
int blkdev_read(unsigned short dev, off_t offset, size_t count, char* buf)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!bh) return -ENOBUFS;   // buffer不够了
        if (i == n - 1)
            len = mod_count;
        else if (i == 2)
            len = BLOCK_SIZE;
